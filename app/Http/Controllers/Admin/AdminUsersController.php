...
class AdminUsersController extends Controller
{
    ...
    public function store(Request $request)
    {
    	...
    	$image_url=ToolsAdmin::uploadFile($params['image_url']);
    		...
    		try {
    			...
    			$adminUsers->addRecord($data);
    			$id=$adminUsers->getMaxId();//获取用户最新的id
    			//添加用户和角色关联关系
    ...
    public function edit($id)
    {
    	...
    	$assign['role_id'] = $userRole ->getByUserId($id)->role_id ?? 0;//获取当前编辑角色的id
    	$assign['user']=AdminUsers::getUserById($id);//获取用互信息
    ...
    public function doEdit(Request $request)
    {
    	...
    	if(!empty($params['image_url'])){
    		...
    		try {
    			...
    			$adminUsers->updateUser($data,$params['id']);
